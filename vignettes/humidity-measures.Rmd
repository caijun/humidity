...
[Saturation vapor pressure](http://oai.dtic.mil/oai/oai?verb=getRecord&metadataPrefix=html&identifier=AD0778316) $e_s$ is calculated from a given temperature $T$ (in $K$) by using the [Clausius-Clapeyron relation](http://en.wikipedia.org/wiki/Clausius%E2%80%93Clapeyron_relation).
\begin{equation}
...
An alternative way to calculate saturation vapor pressure $e_s$ is per the equation proposed by @Murray1967.
\begin{equation}
...
[Absolute humidity](http://en.wikipedia.org/wiki/Humidity#Absolute_humidity) is the total amount of water vapor $m_w$ present in a given volume of air $V$. The definition of absolute humidity can be described as follows
$$
  \rho_w = \frac{m_w}{V}
$$
...
[Mixing ratio](http://www.geog.ucsb.edu/~joel/g266_s10/lecture_notes/chapt03/oh10_3_01/oh10_3_01.html) is the ratio of water vapor mass $m_w$ to dry air mass $m_d$, expressed in equation as follows
$$
  \omega = \frac{m_w}{m_d}
$$
...
[Specific humidity](http://en.wikipedia.org/wiki/Humidity#Specific_humidity) is the ratio of water vapor mass $m_w$ to the total (i.e., including dry) air mass $m$ (namely, $m = m_w + m_d$). The definition is described as
$$
